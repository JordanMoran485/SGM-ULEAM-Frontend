...
export const PhotoPicker = React.memo(function PhotoPicker({
    photo,
    required,
    loading,
    error,
        ...
        <View style={s.photoSection}>
            ...
            {photo ? (
                ...
                >
                    ...
                    <View style={s.photoIconBox}>
                        <MaterialCommunityIcons
                            name={loading ? 'image-sync-outline' : 'image-outline'}
                            size={32}
                            color="rgba(255,255,255,0.9)"
                        />
                    </View>
                    <Text style={s.photoPlaceholderText}>
                        {loading ? 'Procesando imagen...' : 'Sin evidencia adjunta'}
                    </Text>
                </LinearGradient>
            ) : loading ? (
                <View style={s.photoPlaceholderSecondary}>
                    ...
                    <Text style={s.photoPlaceholderSecondaryText}>Procesando imagen...</Text>
                </View>
            ) : (
                <TouchableOpacity activeOpacity={0.85} style={s.photoPlaceholderSecondary} onPress={onTakePhoto}>
                    ...
                    <Text style={s.photoPlaceholderSecondaryText}>
                        {hasPhoto ? 'Cambiar foto' : 'Agregar segunda foto'}
                    </Text>
                </TouchableOpacity>
            )}
            ...
            {(required || photo) && (
                <View style={s.photoActions}>
                    <TouchableOpacity activeOpacity={0.85} style={s.photoBtnSecondary} onPress={onTakePhoto}>
                        <MaterialCommunityIcons name="camera-outline" size={16} color="#4A6CF7" />
                        <Text style={s.photoBtnTextSecondary}>{hasPhoto ? 'Tomar otra' : 'Camara'}</Text>
                    </TouchableOpacity>
                    <TouchableOpacity activeOpacity={0.85} style={s.photoBtnSecondary} onPress={onPickFromLibrary}>
                        ...
                        <Text style={s.photoBtnTextSecondary}>{hasPhoto ? 'Cambiar' : 'Galeria'}</Text>
                    </TouchableOpacity>
                </View>
            )}
        </View>
